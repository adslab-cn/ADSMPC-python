...
from crypten.utils.param_provider import ParamProvider
...
def main():
    ...
    for step, batch in enumerate(eval_dataloader):
        ...
        print(f"--- 第 {step + 1} 批次结果对比 ---")
        for i in range(len(predictions)):
            # .item() 用于从单个元素的Tensor中提取出 Python 数字
            prediction_label = predictions[i].item()
            reference_label = references[i].item()
            is_correct = "正确" if prediction_label == reference_label else "错误"
            print(f"样本 {i+1}: 预测结果 = {prediction_label}, 正确标签 = {reference_label}  => 【{is_correct}】")
        print("---------------------------\n")
