...
class Module:
    ...
    def __call__(self, *args, **kwargs):
        for handle in self._forward_pre_hooks.values():
            # pre_hook 可以修改输入
            result = handle(self, args) 
            if result is not None:
                # 如果 pre_hook 返回了东西，它必须是新的 args (元组) 和 kwargs (字典)
                if isinstance(result, tuple) and len(result) == 2:
                    args, kwargs = result
                else: # 简化：只支持修改 args
                    args = result
        # 1. 首先，正常执行 forward 方法
        result = self.forward(*args, **kwargs)

        # 2. 然后，遍历并执行所有注册的前向钩子
        #    钩子函数签名: hook(module, input, output)
        for handle in self._forward_hooks.values():
            handle(self, args, result) # 注意：这里的 input 是 args 元组
    ...
    def register_forward_pre_hook(self, hook):
        ...
        """
        # (Handle 类的定义与 register_forward_hook 中的完全一样)
        class Handle:
        ...
        hook_id = id(hook)
        handle = Handle(self._forward_pre_hooks, hook_id)
    ...
    def register_forward_hook(self, hook):
        ...
        Returns a handle that can be used to remove the hook by calling
        ``handle.remove()``.
        """
        # 使用一个简单的 Handle 类来管理钩子的移除
        class Handle:
            def __init__(self, hooks_dict, hook_id):
                self.hooks_dict = hooks_dict
                self.id = hook_id
        ...
        # 为钩子生成一个唯一的 ID
        hook_id = id(hook)
        handle = Handle(self._forward_hooks, hook_id)
...
class MatMul(Module):
    ...
    def forward(self, x, mode='secure'):
        if mode == 'shape_inference':
            ...
            # 使用 PyTorch 的规则来推断输出形状
            # 我们创建两个具有输入形状的零张量来计算输出形状
            dummy_input1 = torch.zeros(input1.shape)
            dummy_input2 = torch.zeros(input2.shape)
            output_shape = torch.matmul(dummy_input1, dummy_input2).shape
            
            # 返回一个具有正确形状的虚拟 CrypTensor
            return crypten.cryptensor(torch.zeros(output_shape))
        if hasattr(self, "weight"):
...
class Softmax(Module):
    ...
    def forward(self, input, mode='secure'):
        if mode == 'shape_inference':
            return crypten.cryptensor(torch.zeros(input.shape))
...
class LayerNormalization(Module):
    ...
    def forward(self, x, mode='secure'):
        if mode == 'shape_inference':
            # LayerNormalization 的输出形状与第一个输入的形状完全相同。
            # 我们只需要获取该形状并返回一个虚拟张量。
            assert isinstance(x, (list, tuple)) and len(x) > 0, "LayerNormalization expects at least one input tensor in a list/tuple"
            
            input_tensor = x[0]
            output_shape = input_tensor.shape
            
            # 使用 crypten.cryptensor 工厂函数创建虚拟张量
            return crypten.cryptensor(torch.zeros(output_shape))
...
class ReLUFastSecNet(Module):
    ...
    def __init__(self, approximate="none", inplace=False):
        ...
        if inplace:
            logging.warning("FastSecNet ReLU module does not support inplace computation.")

    def forward(self, x):
        return x.relu_fastsecnet(approximate=self.approximate)
