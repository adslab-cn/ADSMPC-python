...
def _get_operator_class(node_op_type, attributes):
    ...
    """
    if node_op_type == "crypten::ReLU_FastSecNet":
        return module.ReLUFastSecNet
    ...
    if SYM_REGISTRY:
        ...
        for version_key, version_val in sym_registry._registry.items():
            for function_key in version_val.keys():
                ...
                if function_key == "relu":
                    sym_registry._registry[version_key][
                        function_key
                    ] = _onnx_crypten_relu_fastsecnet
    else:
        ...
        torch.onnx.register_custom_op_symbolic(
            "aten::relu", _onnx_crypten_relu_fastsecnet, _OPSET_VERSION
        )
...
@sym_help.parse_args("v")
def _onnx_crypten_relu_fastsecnet(g, input):
    """
    Symbolic function to convert PyTorch's ReLU into a custom ONNX node
    that signifies it should be executed using our FSS protocol.
    """
    # "crypten::ReLU_FSS" 是我们自定义的操作符名称。
    # 'g.op()' 是 PyTorch ONNX exporter 的 API，用于在图中创建一个新节点。
    return g.op("crypten::ReLU_FastSecNet", input).setType(input.type())