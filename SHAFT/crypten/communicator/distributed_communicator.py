...
class DistributedCommunicator(Communicator):
    ...
    def add_provider(self, name, provider_instance):
        """动态地向 Communicator 实例添加一个 Provider。"""
        # 首次调用时，创建一个 _providers 字典来存储所有的 provider
        if not hasattr(self, '_providers'):
            self._providers = {}
        print(f"Attaching provider '{name}' to the global communicator.")
        self._providers[name] = provider_instance

    def get_provider(self, name):
        """从 Communicator 实例中获取一个已添加的 Provider。"""
        if not hasattr(self, '_providers') or name not in self._providers:
            raise RuntimeError(f"Provider '{name}' not found. Was it attached to the communicator?")
