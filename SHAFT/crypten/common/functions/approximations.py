...
from crypten.communicator import Communicator as comm
import pickle
from crypten.common.functions.logic import relu as logic_relu
__all__ = [
    "exp",
    "log",
    "reciprocal",
...
def exp(self):
    ...
    if method == "ideal":
        return ct.cryptensor(torch.exp(self.get_plain_text()), device=self.device)
    elif method == "limit":
...
def reciprocal(self, input_in_01=False):
    ...
    if method == "ideal":
        return ct.cryptensor(torch.reciprocal(self.get_plain_text()), device=self.device)
...
def inv_sqrt(self):
    ...
    if method == "ideal":
        return ct.cryptensor(torch.rsqrt(self.get_plain_text()), device=self.device)
    elif method == "NR":
...
def sigmoid(self):
    ...
    if method == "ideal":
        return ct.cryptensor(torch.sigmoid(self.get_plain_text()), device=self.device)
        ...
        alpha, _, beta_sin = ct.common.util.fourier_series(torch.tanh, width, terms)
...
def tanh(self):
    ...
    if method == "ideal":
        return ct.cryptensor(torch.tanh(self.get_plain_text()), device=self.device)
    if method == "reciprocal":
        return self.mul(2).sigmoid().mul(2).sub(1)
    elif method == "chebyshev":
        terms = cfg.functions.sigmoid_tanh_terms
        coeffs = ct.common.util.chebyshev_series(torch.tanh, 1, terms)[1::2]
        ...
        _, _, beta_sin = ct.common.util.fourier_series(torch.tanh, width, terms)
...
def _fourier_series(self, terms, period, alpha=None, beta_cos=None, beta_sin=None):
    ...
    provider = ct.mpc.get_default_provider()
...
def erf(tensor):
    ...
    if method == "ideal":
        return ct.cryptensor(torch.erf(tensor.get_plain_text()), device=tensor.device)
    elif method == "taylor":
        ...
        _, _, beta_sin = ct.common.util.fourier_series(torch.erf, width, terms)
...
def relu_fastsecnet(self):
    ...
    res = FastSecNetReLU.eval(ArithmeticSecretSharing(RingTensor(self.data)), keys_for_this_layer,my_rank)
    return res
...
def relu(self, approximate="logic_relu", mode='secure'):
    ...
    if approximate=="logic_relu":
        ...
        raise ValueError(f"Unrecognized method {approximate} for relu")
    relu_fastsecnet(self, approximate=approximate)
...
def gelu(self, approximate="none"):
    ...
    if method == "ideal":
        ...
        if approximate == "tanh":
            #_, _, beta_sin = ct.common.util.fourier_series(_diff_gelu_tanh, width, terms)
...
def silu(self):
    ...
    if method == "ideal":
        return ct.cryptensor(torch.nn.functional.silu(self.get_plain_text()), device=self.device)
    elif method == "fourier":
        ...
        #_, _, beta_sin = ct.common.util.fourier_series(_diff_silu, width, terms)
...
def softmax(self, dim,**kwargs):
    ...
    if method == "ideal":
        return ct.cryptensor(torch.softmax(self.get_plain_text(), dim=dim), device=self.device)
    if method == "reciprocal":
