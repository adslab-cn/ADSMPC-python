...
@functools.lru_cache(maxsize=10)
def fourier_series(func, width, terms, step=1e-6):
    r"""Computes Fourier coefficients

    For L = width, A = beta_cos, B = beta_sin, the Fourier series coefficient is

    .. math::
        alpha = 1 / 2L * \int_{-L}^L f(x) dx, 
        A_i = 1 / L * \int_{-L}^L f(x) \cos((n\pi x) / L) dx,
        B_i = 1 / L * \int_{-L}^L f(x) \sin((n\pi x) / L) dx

    Args:
        func (function): function to be approximated
        width (int): approximation will support inputs in range [-width, width]
        terms (int): number of Fourier terms used in approximation
        step (float): interval for numerical integration

    Returns:
        Fourier coefficients with shape equal to num of terms.
    """
    n_range = torch.arange(start=1, end=terms+1).float()
    x = torch.arange(start=-width, end=width, step=step).float()
    npx = np.pi * torch.unsqueeze(n_range, -1) @ torch.unsqueeze(x, 0) / width
    y, cos_npx, sin_npx = func(x), torch.cos(npx), torch.sin(npx)
    alpha = torch.trapz(y, x) / (2 * width)
    beta_cos = torch.trapz(y * cos_npx, x) / width
    beta_sin = torch.trapz(y * sin_npx, x) / width
    return alpha, beta_cos, beta_sin
