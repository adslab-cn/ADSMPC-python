...
# Modified by Andes Y. L. Kei: Updated version number
#
...
def where(condition, input, other):
    ...
    if is_encrypted_tensor(input):
        ...
        return other * (1 - condition) + input * condition
...
def cat(tensors, dim=0):
    ...
    if all(torch.is_tensor(t) for t in tensors):
        try:
            ...
            # TODO: fix this for patch embedding of ViT
            tensors[0] = tensors[0].cuda()
            tensors[1] = tensors[1].cuda()
            return torch.cat([tensors[0], tensors[1]])
...
def save_cache():
    crypten.mpc.get_default_provider().save_cache()

def load_cache():
    crypten.mpc.get_default_provider().load_cache()
